define([
    ...
    return declare('entels.StyledGeoJsonLayer', [L.GeoJSON], {
        ...
        getMarkers: function (markerLayer) {
            var markerlayerId = markerLayer.__id,
                layer,
                _layers,
                markers = [];
            if (markerlayerId && this.layersById[markerlayerId]) {
                layer = this.layersById[markerlayerId];
                _layers = layer._layers;
                for (var layer_key in _layers) {
                    if (_layers.hasOwnProperty(layer_key)) {
                        markers.push(_layers[layer_key]);
                    }
                }
            }
            return markers;
